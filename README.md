...
### 🚀 CyberPUF LLM Edition (Büyük Dil Modeli Sürümü)
Bu projenin Büyük Dil Modellerine (LLM - Llama, Qwen, Phi vb.) uyarlanmış, devasa ağırlık dosyalarını RAM-disk üzerinde (tmpfs) eşzamanlı streaming (AES-GCM) ile çözen **gelişmiş versiyonuna** göz atmak isterseniz aşağıdaki repoya gidebilirsiniz:
...
### 🚀 CyberPUF LLM Edition (Large Language Model Version)
If you are looking for the advanced adaptation of this project tailored for **Large Language Models (LLMs)** (like Llama, Qwen, Phi), which features on-the-fly streaming AES-GCM decryption into a secure RAM-disk (tmpfs) to protect massive weight architectures, please visit the dedicated repository:
👉 [CyberPUF-LLM-Edition Repository](https://github.com/mecik-arda/CyberPUF-LLM-Edition)
